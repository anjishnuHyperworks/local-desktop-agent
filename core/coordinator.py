...
class Coordinator(QObject):
    ...
    def create_plan(self, goal: str, prior_context: Optional[str] = None) -> Plan:
        ...
        planner_prompt = (
            ...
            "Each task is a meaningful sub-goal that the executor accomplishes by observing the "
            "screen and taking AS MANY actions as it needs (multiple clicks, types, scrolls, or "
            "presses).\n"
            "Group related actions into ONE task. A sequence of clicks on the same app/window "
            "that achieves one outcome is a single task — NOT one task per click or keystroke. "
            "Each task you create costs the executor an extra screenshot to confirm completion, "
            "so over-decomposing directly wastes time and money.\n"
            ...
            "Be specific but concise, and prefer fewer, broader tasks. Examples:\n"
            "Goal 'add 2 and 3 on the calculator' →\n"
            "1. Open the Calculator app\n"
            "2. Enter 2 + 3 = and read the result\n"
            "Goal 'look up a spreadsheet in Google Sheets' →\n"
            "1. Open Chrome and navigate to Google Sheets\n"
            "2. Open the spreadsheet and inspect it\n"
        )
